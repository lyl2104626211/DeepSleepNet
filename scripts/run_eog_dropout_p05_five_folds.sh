...
RESULT_ROOT=${RESULT_ROOT:-results/deepsleepnet_sleep_edf_paper_fpz_cz_eeg_eog_dropout_p05}
...
for fold in "${FOLD_LIST[@]}"; do
  ...
  echo "==> ${fold}: train stage1 with EOG dropout p=${EOG_DROPOUT_PROB}"
  ...
  echo "==> ${fold}: train stage2 with EOG dropout p=${EOG_DROPOUT_PROB}"
