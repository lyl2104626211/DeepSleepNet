...
from eeg_sleep.robust_schemes.scheme_d_quality_guided_generator import RuleBasedQualitySensor


class QualityGuidedGeneratorV2FeatureNet(nn.Module):
    """Quality-guided EOG feature recovery v2.

    Compared with v1, v2 keeps the same quality sensor and EEG-to-EOG
    generator, but reduces generator supervision and makes EOG contribution
    learnable:

      H_eog_final = c * H_eog_real + (1 - c) * H_eog_fake
      H_fusion = H_eeg + alpha * H_eog_final

    The goal is to reduce normal-condition interference while keeping the
    missing-EOG recovery path.
    """
    ...
    def __init__(
        ...
        n_classes: int = 5,
        dropout: float = 0.5,
        generator_loss_weight: float = 0.01,
        eog_residual_init: float = 0.5,
        ...
        init = min(max(float(eog_residual_init), 1e-4), 1.0 - 1e-4)
        self.eog_residual_logit = nn.Parameter(torch.tensor(math.log(init / (1.0 - init))))
    ...
    def _split_inputs(inputs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if inputs.ndim != 3 or inputs.shape[1] < 2:
            raise ValueError("QualityGuidedGeneratorV2FeatureNet expects [B, 2, L] or more channels")
    ...
    def extract_features_with_aux(self, inputs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        eog_residual_scale = torch.sigmoid(self.eog_residual_logit)
        fused_features = eeg_features + eog_residual_scale * eog_final
...
class QualityGuidedGeneratorV2DeepSleepNet(nn.Module):
    """Stage2 DeepSleepNet variant with quality-guided EOG feature recovery v2."""
    ...
    def __init__(
        ...
        self.feature_extractor = QualityGuidedGeneratorV2FeatureNet(
            input_size=input_size,
            n_classes=n_classes,
            dropout=feature_dropout,
            generator_loss_weight=generator_loss_weight,
            eog_residual_init=eog_residual_init,
    ...
    def forward(self, inputs: torch.Tensor):
        if inputs.ndim != 4:
            raise ValueError(f"QualityGuidedGeneratorV2DeepSleepNet expects [B, S, C, L], got {tuple(inputs.shape)}")

        batch_size, sequence_length, input_channels, signal_length = inputs.shape
        if input_channels < 2:
            raise ValueError("QualityGuidedGeneratorV2DeepSleepNet requires EEG+EOG input")
